...
class Graph:
    ...
    def dfs_recursive(self, starting_vertex, target_vertex, visited=None, path=None):
        ...
        This should be done using recursion.
        """
        # Initiate visited as a set (unordered contents)
        if visited is None:
            visited = set()
        # Initiate the path as an array (ordered contents)
        if path is None:
            path = []
        # Adding content to visited
        visited.add(starting_vertex)
        # Add the vertex to the path
        path = path + [starting_vertex]
        # If the starting vertex is the same as the target vertex:
        if starting_vertex == target_vertex:
            #return the path
            return path
        # Else: call dsf_recursive on each unvisited neighbor
        for neighbor in self.get_neighbors(starting_vertex):
            if neighbor not in visited:
